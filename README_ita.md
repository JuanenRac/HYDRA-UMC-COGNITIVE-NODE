...
> **Verifica di onestà - cosa funziona davvero oggi:** il controllo di disponibilità della famiglia e il suo schema JSON versionato (`family.py`, `manifest.py`, `models.py`) e la semplice superficie HTTP della stdlib (`api.py`) sono reali e coperti da 33 test superati (`pytest tests/` - `test_manifest.py`, `test_models.py`, `test_family.py`, `test_api.py`, `test_cli.py`). Questo nodo non esegue alcun modello da solo: "Esecuzione locale LLM", "Integrazione VLA" e "Elaborazione comandi vocali" nelle Funzionalità Chiave qui sotto sono esplicitamente contrassegnate come `(pianificato)` - non esiste codice runtime Hailo-10 in questo repository. `docker-compose.yml` ora fa riferimento a un vero Dockerfile per tutti e 4 i figli, ma non è mai stato effettivamente eseguito come un unico stack in questo ambiente di sviluppo (nessun runtime Docker disponibile qui) - la mappatura comando/porta di ogni Dockerfile è stata verificata a mano rispetto a ciò che la propria unità systemd di ogni figlio esegue già sulla vera CM5, non verificata dal vivo. Vedi `CHANGELOG.md` per cosa è stato esattamente consegnato finora.
...
* **Perché `docker-compose.yml` documenta il contratto di integrazione,
  non solo collega le immagini.** Decidere quale servizio dipende da
  quale e quali mount di dispositivo/volume serve a ciascuno è stato
...
│   └── main.py                        # Punto di ingresso + sottocomandi reali `family-status [--json]` e `serve`
├── tests/                          # Test reali: lettura manifest, modelli, stato famiglia, api, CLI end-to-end
├── docs/
│   └── CLI_REFERENCE.md            # Riferimento completo dei comandi: ogni flag, output reale catturato, codici di uscita
...
Voice-UI, Semantic-Planner, Docs-QA) si collegano a questo nodo - ognuno
ha ora un proprio vero Dockerfile.
...
`serve` esegue lo stesso identico controllo `family-status` come un
piccolo `http.server` della libreria standard invece di un'unica
chiamata CLI - è il comando reale che l'unit systemd
`hydra-umc-cognitive-node.service` della CM5 esegue in produzione:

```bash
./run.sh serve --addr 127.0.0.1 --port 8096
# GET /family-status  -> lo stesso JSON stampato sopra da `family-status --json`
# GET /stats          -> { "workspace": "<workspace predefinito configurato>" }
```

`GET /family-status` accetta un `?workspace=` opzionale per sovrascriverlo;
qualsiasi altro percorso restituisce `404`. Vedi il
[Riferimento CLI](docs/CLI_REFERENCE.md) per il riferimento completo dei
comandi: ogni flag, l'output reale catturato di `-h`/`curl` e la tabella
...
* **`docker compose up` non riesce ad avviare `voice-ui`.** Il suo
  container rifiuta di ascoltare oltre loopback senza un vero
  `HYDRA_UMC_VOICE_UI_TOKEN` (di proposito - vedi il commento dello
  stesso `docker-compose.yml`); impostalo in un vero file `.env` o nel
  tuo gestore di segreti, non commetterlo mai. Eseguire ogni servizio
  direttamente con il proprio `run.sh`/`run.bat` durante lo sviluppo non
  richiede questo token, perché lì `--host` resta su loopback.
